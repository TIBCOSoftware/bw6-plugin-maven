...
public class BWEARPackagerMojo extends AbstractMojo {
	...
	// This map is required for maintaining the module name vs module version
	// which needs to be updated in the TibcoXML at a later stage.
	...
	public void execute() throws MojoExecutionException {
		try {
			...
			throw new MojoExecutionException(
					"Failed to create BW EAR Archive ", e1);
		}
	}
	...
	/**
	 * Adds the Modules included in the Application to the EAR file. It will
	 * also maintain a Module vs Version map which will be used later by the
	 * Application to populate the TIBCO.xml
	 * 
	 * @throws Exception
	 */
	private void addModules() throws Exception {
		try {
			...
			this.version = manifest.getMainAttributes().getValue(
			...
			for (Artifact artifact : artifacts) {
				...
				if (mf.getMainAttributes().containsKey("TIBCO-BW-SharedModule")) {
					jarchiver.addFile(moduleJar, artifact.getArtifactId() + "_"
				...
				getLog().info(
						"Adding Module JAR with name " + moduleJar.getName()
			...
			// This code allows dependencies declared in a Module to make it to
			// the root level of the ear file
			...
			for (MavenProject project : projects) {
				...
				for (Artifact artifact : dependencyArtifacts) {
					...
					String dependencyVersion = BWProjectUtils
							.getModuleVersion(artifact.getFile());
					...
					for (Object str : mf.getMainAttributes().keySet()) {
						...
						if ("TIBCO-BW-SharedModule".equals(str.toString())) {
							moduleVersionMap.put(artifact.getArtifactId(),
									dependencyVersion);
			...
			// This code takes dependencies in the application project and adds
			// them to the EAR file root level
			...
			if (resolutionResult != null) {
				for (Dependency dependency : resolutionResult.getDependencies()) {
					...
					if (moduleVersionMap.containsKey(dependency.getArtifact()
							.getArtifactId())) {
						continue;
					}

					Manifest mf = ManifestParser
							.parseManifestFromJAR(dependency.getArtifact()
									.getFile());
					...
					if (mf != null) {
						for (Object str : mf.getMainAttributes().keySet()) {
							...
							if ("TIBCO-BW-SharedModule".equals(str.toString())) {
								String dependencyVersion = BWProjectUtils
										.getModuleVersion(dependency
												.getArtifact().getFile());
								moduleVersionMap.put(dependency.getArtifact()
										.getArtifactId(), dependencyVersion);
								artifactFiles.add(dependency.getArtifact()
										.getFile());
								break;
	...
	private DependencyResolutionResult getDependenciesResolutionResult() {
		...
		try {
			getLog().debug(
					"Looking up dependency tree for the current project => "
							+ project + " and the current session => "
							+ session);
			DefaultDependencyResolutionRequest resolution = new DefaultDependencyResolutionRequest(
					project, session.getRepositorySession());
			resolutionResult = resolver.resolve(resolution);
		} catch (DependencyResolutionException e) {
			getLog().debug(
					"Caught DependencyResolutionException for the project => "
	...
	protected String[] getPluginsToIgnore() {
		if (pluginsToIgnore == null) {
			pluginsToIgnore = new String[] { "com.tibco.bw.palette.shared",
					"com.tibco.xml.cxf.common", "tempbw" };
		}
	...
	/**
	 * Returns the Archive file name and location. The Archive file is created
	 * in the Target directory with the name same as application project which
	 * is also the artifactId for the Application project.
	 * 
	 * @return
	 */
	private File getArchiveFileName() {
		Version version = VersionParser.parseVersion(manifest
				.getMainAttributes().getValue(Constants.BUNDLE_VERSION));
		String fullVersion = version.getMajor() + "." + version.getMinor()
				+ "." + version.getMicro();

		String archiveName = project.getArtifactId() + "_" + fullVersion
				+ ".ear";
		File archiveFile = new File(outputDirectory, archiveName);

		getLog().info(
				"The EAR file name for Application is "
						+ archiveFile.toString());
		return archiveFile;
	}

	/**
	 * Adds the from the META-INF folder to the EAR file. The META-INF folder
	 * needs to be copied as it is.
	 * 
	 * @param metainf
	 *            the META-INF folder location for the Application project.
	 * 
	...
	private File addFiletoEAR(File metainf) throws Exception {
		...
		for (int i = 0; i < fileList.length; i++) {
			if (fileList[i].getName().indexOf("MANIFEST") != -1) { // If the
																	// File is
																	// MANIFEST.MF
																	// then the
																	// Version
				...
				jarchiver.addFile(manifestFile,
						"META-INF/" + fileList[i].getName());
			} else if (fileList[i].getName().indexOf("TIBCO.xml") != -1) { // If
																			// the
																			// File
																			// is
																			// TIBCO.xml
				...
				jarchiver
						.addFile(tibcoXML, "META-INF/" + fileList[i].getName());
			} else if (fileList[i].getName().indexOf(".substvar") != -1) { // The
																			// substvar
																			// files
																			// need
																			// to
	...
	private File getUpdatedManifest(File manifestFile) throws Exception {
		...
		getLog().debug(
				"Manifest added to temp location at " + tempManifest.toString());
		return tempManifest;
	}
	...
	// *
	// * @param target the Module Output directory which is the target
	// directory.
	// *
	...
	// if(files.length == 0) {
	// throw new
	// Exception("Module is not built yet. Please check your Application PO for the Module entry.");
	// }
	// return files[0];
	// }
	 ...
	 * 
	 * @param tibcoxML
	 *            the Application Project TIBCO.xml file
	 * 
	...
	private Document updateTibcoXMLVersion(Document doc) throws Exception {
		...
		NodeList nList = doc.getElementsByTagNameNS(
				Constants.PACKAGING_MODEL_NAMESPACE_URI, Constants.MODULE);
		for (int i = 0; i < nList.getLength(); i++) {
			Element node = (Element) nList.item(i);
			// The Symbolic name is the Module name. The version for this needs
			// to be updated under the tag technologyVersion
			NodeList childList = node.getElementsByTagNameNS(
					Constants.PACKAGING_MODEL_NAMESPACE_URI,
					Constants.SYMBOLIC_NAME);
			String module = childList.item(0).getTextContent();
			NodeList technologyVersionList = node.getElementsByTagNameNS(
					Constants.PACKAGING_MODEL_NAMESPACE_URI,
					Constants.TECHNOLOGY_VERSION);
			Node technologyVersion = technologyVersionList.item(0);
			// Get the version from the Module from the Map and set it in the
			// Document.
	...
	private void updateManifestVersion() {
		String version = manifest.getMainAttributes().getValue(
				Constants.BUNDLE_VERSION);
		String qualifierVersion = VersionParser
				.getcalculatedOSGiVersion(version);
		getLog().info(
				"The OSGi verion is " + qualifierVersion
						+ " for Maven version of " + version);
